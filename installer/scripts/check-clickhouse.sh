#!/bin/bash
# =============================================================================
# Скрипт проверки состояния ClickHouse
# =============================================================================
# Назначение: Проверка состояния и работоспособности ClickHouse
...
# CLICKHOUSE_HTTP_PORT, CLICKHOUSE_TCP_PORT, CLICKHOUSE_CONTAINER_NAME и другие
...
Примеры:
    $0 --format json
    $0 --http-port 9123 --container-name my-clickhouse
...
# Проверка контейнера ClickHouse
check_container() {
    if [[ "$DRY_RUN" == "true" ]]; then
        print_info "[DRY-RUN] Проверка контейнера ClickHouse"
        return 0
    fi
    
    if ! run_sudo docker ps --filter "name=$CLICKHOUSE_CONTAINER_NAME" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -q "$CLICKHOUSE_CONTAINER_NAME"; then
        print_error "Контейнер ClickHouse не запущен"
        return 1
    fi
    
    local container_status=$(run_sudo docker ps --filter "name=$CLICKHOUSE_CONTAINER_NAME" --format "{{.Status}}")
    if echo "$container_status" | grep -q "Up"; then
        print_success "Контейнер ClickHouse запущен: $container_status"
    else
        print_error "Контейнер ClickHouse не активен: $container_status"
        return 1
    fi
...
check_ports() {
    if [[ "$DRY_RUN" == "true" ]]; then
        print_info "[DRY-RUN] Проверка портов ClickHouse"
        return 0
    fi
    
    # Проверка HTTP порта
    if run_sudo netstat -tlnp 2>/dev/null | grep -q ":$CLICKHOUSE_HTTP_PORT "; then
        print_success "HTTP порт $CLICKHOUSE_HTTP_PORT открыт"
    else
        print_error "HTTP порт $CLICKHOUSE_HTTP_PORT не открыт"
        return 1
    fi
    
    # Проверка TCP порта
    if run_sudo netstat -tlnp 2>/dev/null | grep -q ":$CLICKHOUSE_TCP_PORT "; then
        print_success "TCP порт $CLICKHOUSE_TCP_PORT открыт"
    else
        print_error "TCP порт $CLICKHOUSE_TCP_PORT не открыт"
        return 1
    fi
...
# Проверка API ClickHouse
check_api() {
    if [[ "$DRY_RUN" == "true" ]]; then
        print_info "[DRY-RUN] Проверка API ClickHouse"
        return 0
    fi
    
    # Проверка ping endpoint
    if curl -fsS "http://localhost:$CLICKHOUSE_HTTP_PORT/ping" | grep -q 'Ok'; then
        print_success "API ClickHouse доступен (ping)"
    else
        print_error "API ClickHouse недоступен (ping)"
        return 1
    fi
    ...
    if [[ -n "$version_response" ]]; then
        print_success "Версия ClickHouse: $version_response"
    else
        print_warning "Не удалось получить версию ClickHouse"
    fi
    
    rm -f /tmp/clickhouse_ping.txt
    return 0
}
...
check_directories() {
    if [[ "$DRY_RUN" == "true" ]]; then
        print_info "[DRY-RUN] Проверка директорий ClickHouse"
        return 0
    fi
...
check_docker_compose() {
    ...
    local compose_file="$CLICKHOUSE_BASE_DIR/docker-compose.yml"
...
check_resources() {
    if [[ "$DRY_RUN" == "true" ]]; then
        print_info "[DRY-RUN] Проверка ресурсов ClickHouse"
        return 0
    fi
    
    # Проверка использования CPU и памяти
    local stats=$(run_sudo docker stats --no-stream --format "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}" "$CLICKHOUSE_CONTAINER_NAME" 2>/dev/null | tail -n +2)
    ...
    local data_size=$(run_sudo du -sh "$CLICKHOUSE_DATA_DIR" 2>/dev/null | cut -f1)
    local logs_size=$(run_sudo du -sh "$CLICKHOUSE_LOGS_DIR" 2>/dev/null | cut -f1)
...
output_text() {
    print_header "=================================================================="
    print_header "=== Проверка состояния ClickHouse ==="
    print_header "=================================================================="
    echo
    ...
    print_header "=================================================================="
    print_success "Проверка ClickHouse завершена"
    print_header "=================================================================="
}
...
main() {
    ...
    local log_file="$LOG_DIR/check-clickhouse-$(date +%Y%m%d_%H%M%S).log"
    init_logging "$log_file" "$LOG_LEVEL"
    
    log_info "Начало проверки ClickHouse"
    log_info "HTTP порт: $CLICKHOUSE_HTTP_PORT"
    log_info "TCP порт: $CLICKHOUSE_TCP_PORT"
    log_info "Контейнер: $CLICKHOUSE_CONTAINER_NAME"
    ...
    log_info "Проверка ClickHouse завершена"
